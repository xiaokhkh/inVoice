...
| 其他 | `QMI8658` IMU + `PCF85063` RTC | RTC 驱动昼夜显示；IMU 暂未使用 |
...
- 屏幕短按一次会直接发送标准 Return，可用于 Codex、浏览器、终端和聊天输入框；
  去抖后按住 250 ms 则启动语音。
  BOOT 仍是单击打开剪贴板。
- 固件正常运行时按 BOOT 只切换剪贴板面板，不启动录音或音量环。
- 自然人声和主机单独打开麦克风不会触发圆环或 F13。
- PWR 长按关机动作被关闭，避免长句录音时断电。
- 透明 Jam 动画下方的背景色和 AMOLED 亮度按本地时间连续变化：凌晨深蓝、
  清晨暖杏、白天米白、傍晚暖粉、夜间藏青。板载 `PCF85063` 每 30 秒读取
  一次时间，断开 Mac 后仍可保持昼夜变化；RTC 无效时回退为原日间米白色。
...
- OTA 重启完成后会自动用 Mac 本地时间校准 RTC；也可单独运行
  `tools/voiceops-ota/build/voiceops-ota sync-time` 校时。
