...
| 项目 | 分支 / 提交 | 与本计划相关的做法 |
| --- | --- | --- |
| inVoice | `main` / `c9880e9`，本地 HEAD 与 `origin/main` 一致 | 当前工作区包含尚未提交的板端集成和输入链路修改 |
...
- [x] 保留 `ClipboardObserver.markInternalWrite()`，避免内部写入污染 inVoice 剪贴板历史。
...
4. **测试与日志**：先跑单元测试，再构建 Release，最后安装到 `/Users/aminer/Applications/inVoice.app` 做板端回归。
