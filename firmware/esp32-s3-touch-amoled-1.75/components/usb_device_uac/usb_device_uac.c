...
typedef struct {
    ...
    int8_t mute[UAC_MAX_CHANNELS + 1];                            // Feature unit, +1 for master channel 0
    int16_t volume[UAC_MAX_CHANNELS + 1];                         // Feature unit, +1 for master channel 0
...
static inline void notify_event(uac_device_event_t event)
{
    if (s_uac_device != NULL && s_uac_device->user_cfg.event_cb != NULL) {
        s_uac_device->user_cfg.event_cb(event, s_uac_device->user_cfg.cb_ctx);
    }
}
                                   ...
                                   bool return_pressed)
{
    ...
    size_t key_count = 0;
    if (voice_pressed) {
        keycodes[key_count++] = HID_KEY_F13;
    }
...
void tud_mount_cb(void)
{
    ...
    notify_event(UAC_DEVICE_EVENT_MOUNTED);
}
...
void tud_umount_cb(void)
{
    ...
    notify_event(UAC_DEVICE_EVENT_UNMOUNTED);
}
...
void tud_suspend_cb(bool remote_wakeup_en)
{
    ...
    notify_event(UAC_DEVICE_EVENT_SUSPENDED);
}
...
void tud_resume_cb(void)
{
    ...
    notify_event(UAC_DEVICE_EVENT_RESUMED);
}
#endif
...
bool tud_audio_set_itf_close_EP_cb(uint8_t rhport, tusb_control_request_t const *p_request)
{
    ...
    if (s_uac_device->mic_itf_num == itf && alt == 0) {
        ...
        s_uac_device->mic_active = false;
        notify_event(UAC_DEVICE_EVENT_MIC_CLOSED);
    }
#endif
...
bool tud_audio_set_itf_cb(uint8_t rhport, tusb_control_request_t const *p_request)
{
    ...
    if (s_uac_device->mic_itf_num == itf && alt != 0) {
        ...
        notify_event(UAC_DEVICE_EVENT_MIC_OPENED);
    }
#endif
